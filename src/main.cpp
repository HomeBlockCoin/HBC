...
CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // 0,000244140625 proof-of-work difficulty
CBigNum bnProofOfStakeLimit(~uint256(0) >> 20);
CBigNum bnProofOfWorkLimitTestNet(~uint256(0) >> 16);

unsigned int nTargetSpacing = 5 * 60; //Block  spacing 5 minute
unsigned int nStakeMinAge = 24 * 60 * 60 * 5; //Minimum stake age 5 days
unsigned int nStakeMaxAge = 24 * 60 * 60 * 30; //Maximum stake age 30 days
...
const string strMessageMagic = "HomeBlockCoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    int64_t nSubsidy = 10 * COIN; // POW reward

    if(nBestHeight == 1)
    {
		nSubsidy = 8000000 * COIN; // ICO premine
    }
		else
    {
        nSubsidy = 10 * COIN;
...
//const int YEARLY_BLOCKCOUNT = 518400; //1440 * 360
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
    ...
    printf("Block Number %d \n",nBestHeight);
        ...
        if(nBestHeight < 130000)
    	    nRewardCoinYear = 120 * COIN_YEAR_REWARD; //120%
    	else if(nBestHeight < (260000))
    	    nRewardCoinYear = 96 * COIN_YEAR_REWARD; //90%
    	else if(nBestHeight < (520000))
    	    nRewardCoinYear = 84 * COIN_YEAR_REWARD; //84%
    	else if(nBestHeight < (650000))
    	    nRewardCoinYear = 60 * COIN_YEAR_REWARD; //60%
    	else if(nBestHeight < (780000))
            nRewardCoinYear = 36 * COIN_YEAR_REWARD; //36%
		else
			nRewardCoinYear = 16.8 * COIN_YEAR_REWARD; //16.8%
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "HomeBlockCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "HBC Blockchain - Genesis Launch 11/17";
        CTransaction txNew;
        txNew.nTime = 1509967233;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
		...
		*/
        ...
        assert(block.hashMerkleRoot == uint256("0xfc57f5fbd99f1b1d5236dd7291f12113e12ee887070a0d14837225924dddb2f9"));
...
unsigned char pchMessageStart[4] = { 0x12, 0x5c, 0x3f, 0x84 };
